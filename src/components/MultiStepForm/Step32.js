...
const Step32 = props => {
    ...
    const [mapLocation, setMapLocation] = useState(state.interpreterMailAddr_latlng || '');

    useEffect(() => {
        if (props.state.interpreterMailAddr_latlng) {
            setMapLocation(props.state.interpreterMailAddr_latlng);
        }
    }, []);
    ...
    const onChange = (e, fieldName, value) => {
        ...
        switch (fieldName) {
            case 'address':
                updatedState['interpreterMailAddr'] = value;
                updatedState['interpreterMailAddr_latlng'] = value.latLng;
                setErrorMsg(fieldValidator('street', updatedState.interpreterMailAddr, 'required', errors).error);
                break;
            default:
                break;
        }
    ...
    const onNext = () => {
        let errors = {...errorMsg};
        let newForm = fieldValidator('street', state.interpreterMailAddr, 'required', errors);
    ...
    return (
        <Step heading={`Interpreter's Mailing Address`}>
            <img
                src={BackBtn}
                className="btn-back"
                alt={`Back`}
            ...
            <MDBContainer>
                <MDBRow center>
                    <MDBCol sm={`10`} md={`8`}>
                        ...
                        <MDBRow
                            style={{
                                display: state.interpreterMailAddr_latlng ? "block" : "none",
                            }}
                        >
                            <Map
                        ...
                        <MDBRow center>
                            <MDBCol>
                                <GooglePlacesAutocomplete
                                    ...
                                    initialValue={(state.interpreterMailAddr && state.interpreterMailAddr.street) || ''}
